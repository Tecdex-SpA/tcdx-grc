...
type SeedManifest = { permissionRows: number; lifecycleEdges: number; configurationDefaults: number; protectedRegulatoryContents: number; contentSha256: string };
...
try {
  ...
  const counts = {
    ...
    lifecycleEdges: await scalar("SELECT count(*)::text AS count FROM ops_audit.lifecycle_transition_definitions"),
  ...
  const expected = { plans: 3, capabilities: 20, entitlements: 36, roles: 24, permissions: manifest.permissionRows, lifecycleEdges: manifest.lifecycleEdges, configurationDefaults: manifest.configurationDefaults, regulatoryPackHeaders: 5, regulatoryPackVersions: manifest.protectedRegulatoryContents, impactLevels: 5, likelihoodLevels: 5, methodologies: 1 };
  ...
  const dismiss = await client.query<{ from_state: string; to_state: string; command_code: string }>("SELECT from_state, to_state, command_code FROM ops_audit.lifecycle_transition_definitions WHERE entity_type='Issue' AND to_state='dismissed' ORDER BY from_state");
