...
| 11 | exact permissions and grants | Permission/Role/RolePermission | 05/09 | CLOSED: 136 runtime rows after exactly two PRE-F5C additions; 147 executable permission contracts; default DENY |
...
| 15 | per-operation/transition idempotency | same | 03/07/09 | CLOSED: 80 POST operations key-required; 26 GET operations naturally idempotent |
| 16 | audit envelope/redaction/conditional tenant | `ops_audit.audit_events` | 08 | CLOSED |
| 17 | exact audit codes/mappings | same | 03/08/09 | CLOSED: 154 unique codes derived from 80 operation mappings and 100 lifecycle mappings; API-backed lifecycle commands share exactly one code |
...
| 31 | exact lifecycle edges/policies/events | `ops_audit.lifecycle_transition_* ` | 02/03/05/08/09/10 | CLOSED: 100 authoritative edges; PRE-F5C graph reachability is complete; exactly two `Issue -> dismissed` rows remain internal; all absent edges DENY |
...
```text
PUBLIC_API_OPERATIONS=106
PUBLIC_READ_OPERATIONS=26
PUBLIC_MUTATING_OPERATIONS=80
PUBLISHED_DOMAIN_INTEGRATION_EVENTS=56
PUBLISHED_EXECUTABLE_PERMISSION_CONTRACTS=147
