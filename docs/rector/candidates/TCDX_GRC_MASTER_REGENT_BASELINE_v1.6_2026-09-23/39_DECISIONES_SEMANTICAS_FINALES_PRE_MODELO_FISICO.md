...
## 19. Cierre PRE-F5E Platform/IAM

- `Tenant.initial_lifecycle_state=active`; el valor es server-owned.
- `Tenant.data_classification` usa exclusivamente el vocabulario versionado Platform `public | internal | confidential | restricted`.
- `Tenant.initial_data_classification=confidential`; el caller de `tenantCreate` no controla ese valor inicial.
- `TenantMembership.initial_membership_state=active`; el valor es server-owned.
- crear `TenantMembership` para una `UserIdentity` existente no crea invitación, identidad, credencial ni rol.
- `PlatformRoleAssignment` materializa el grant humano Platform persistido, temporal y auditable, exclusivamente hacia `Role(PLATFORM_CONTROL)`, sin tenant ni membership.
- autoridad Platform y autoridad tenant siguen las dos rutas cerradas de 09/22 y terminan siempre en default DENY cuando falta cualquier eslabón.
- `FIRST_PLATFORM_ADMIN_BOOTSTRAP` puede crear exclusivamente el primer `PlatformRoleAssignment` abierto para la `UserIdentity` humana autenticada y el Role canónico `PLATFORM_ADMIN`; no acepta un `role_id` arbitrario ni requiere tenant o TenantMembership.
- la ceremonia es interna, one-time, transaccional y auditable: serializa sobre la fila canónica `PLATFORM_ADMIN`, exige cero assignments activos y cero filas históricas, crea assignment + `audit.iam.platform_role_assignment.bootstrap.v1` atómicamente y todo segundo intento es DENY.
- no existe endpoint público, flag de reactivación, allowlist de identidad, seed personal, tabla bootstrap ni autoridad persistente distinta de `PlatformRoleAssignment`; la implementación runtime posterior no puede debilitar estas precondiciones.
